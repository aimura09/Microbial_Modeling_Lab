...
options = optimoptions('ga','ConstraintTolerance',1e-4, 'UseParallel',true, 'UseVectorized', true,...
                       'CrossoverFraction',0.85,'PopulationSize',100, 'StallGen', 12,'Generations',1000);
...
model = addMetabolite(model, 'lactate_e', ...
                'metName', 'pseudo-met', ...
                'metFormula', '', ...
                'Charge', 0, ...
                'csense', 'E');
...
% add pseudo metabolite
model = addReaction(model, 'Pseudo_Lactate', 'metaboliteList', {'lactate_e'}, 'stoichCoeffList', [-1]);

% change objective function to the new pseudo lactate reaction
model = changeObjective(model,{'Pseudo_Lactate'});
...
for i = 0.1:1:1
    ...
    for j = 0.1:1:1
        ...
        lb = (model.lb);
        ub = (model.ub);
        func = @(v) objective_infile(v, c); % removed the negative after adding pseudo lactate variable, since we are actually trying to minimize it
        tic;
        ...
        if max_ex_dlac < dlactate(y,x)
            ...
            % display the updated lactate %

            % display EX_Dlactate_D[e] value
            disp(model.rxnNames(866)) % 866 = EX_Dlactate_D[e] index
            disp(max_ex_dlac)

            disp("\n")

            % display pseudo lactate value
            disp(model.rxnNames(869)) % 869 = pseudo lactate index
            disp(max_ex_dlac_flux(869))
        end
...
function f = objective_infile(v, c)
    f = zeros(1,size(v,1));
    for i = 1:size(v,1)
        % assign the flux of the pseudo lactate reaction to the negative of EX_Dlactate_D[e]
        v(i,869) = -v(i,866);
