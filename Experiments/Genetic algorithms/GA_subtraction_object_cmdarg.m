...
function GA_subtraction_object_cmdarg(args1,args2,args3,args4,args5,args6,args7)
output_folder = args1; 
...
folder_path = ['output folder : output/' output_folder];
disp(folder_path);
load('iJN678.mat')
model = iJN678;
...
disp(['objective rxn : ' objective_rnx])
if strcmp(objective_rnx,'EX_Ethylene_D')
    model = changeObjective(model,{'EX_Ethylene_D[e]'},[1]);
end
if strcmp(objective_rnx,'EX_Dlactate_D')
    model = changeObjective(model,{'EX_Dlactate_D[e]'},[1]);
end
if strcmp(objective_rnx,'both')
